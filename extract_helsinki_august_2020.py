...
print("Filtering for August 2020...")
august_2020 = df[
    (df['departure'].dt.year == 2020) & 
    (df['departure'].dt.month == 8)
].copy()

print("\nInitial data quality check:")
print(f"Total records: {len(august_2020):,}")

# Check for missing values
print("\nMissing values:")
print(august_2020.isnull().sum())

# Check for invalid values
print("\nChecking for invalid values...")
print("Duration statistics (seconds):")
print(august_2020['duration (sec.)'].describe())

print("\nDistance statistics (meters):")
print(august_2020['distance (m)'].describe())

print("\nSpeed statistics (km/h):")
print(august_2020['avg_speed (km/h)'].describe())

# Basic data cleaning
print("\nCleaning data...")

# 1. Remove rides with invalid duration (less than 10 seconds or more than 24 hours)
valid_duration = (august_2020['duration (sec.)'] >= 10) & (august_2020['duration (sec.)'] <= 86400)
...
valid_distance = (august_2020['distance (m)'] >= 10) & (august_2020['distance (m)'] <= 100000)
...
valid_speed = (august_2020['avg_speed (km/h)'] > 0) & (august_2020['avg_speed (km/h)'] <= 50)
invalid_speed_count = (~valid_speed).sum()
print(f"Rides with invalid speed: {invalid_speed_count:,}")

# Apply all filters
clean_data = august_2020[valid_duration & valid_distance & valid_speed]

print(f"\nRecords after cleaning: {len(clean_data):,}")
print(f"Removed {len(august_2020) - len(clean_data):,} invalid records")

# Save cleaned dataset
output_file = 'Helsinki_August_2020_cleaned.csv'
